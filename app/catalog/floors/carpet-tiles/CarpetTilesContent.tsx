...
type Props = {
  shawCollections: CarpetTileCollection[];
  fomatCollections: CarpetTileCollection[];
};
...
export default function CarpetTilesContent(props: Props) {
  return (
    <Suspense fallback={null}>
      <CarpetTilesPageContent {...props} />
    </Suspense>
  );
}

function CarpetTilesPageContent({ shawCollections, fomatCollections }: Props) {
  ...
  const collections = active === "shaw" ? shawCollections : fomatCollections;
    ...
    <div>
      ...
      {collections.length > 0 && (
        <section className="py-20 bg-[#fafaf8] border-t border-[#e6e4d8]">
          <div ref={collectionsRef.ref} className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className={`mb-10 transition-all duration-700 ${collectionsRef.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
              <p className="text-[#8c6d51] text-xs font-semibold tracking-[0.2em] uppercase mb-3">Коллекции</p>
              <h2 className="text-3xl font-semibold text-[#22333b] tracking-tight" style={{ fontFamily: "var(--font-montserrat)" }}>
                {isShaw ? "Линейка Shaw Contract" : "Линейка FOMAT"}
              </h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {collections.map((c, i) => (
                <div
                  key={c._id}
                  className={`transition-all duration-700 ${collectionsRef.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
                  style={{ transitionDelay: `${i * 80}ms` }}
                >
                  {c.images?.length ? (
                    <AutoSlider images={c.images} aspect="aspect-square" delay={5000} />
                  ) : (
                    <div className="aspect-square bg-[#22333b] rounded-sm" />
                  )}
                  <p className="mt-3 text-[#22333b] text-sm font-medium tracking-wide" style={{ fontFamily: "var(--font-montserrat)" }}>
                    {c.name}
                  </p>
                </div>
              ))}
